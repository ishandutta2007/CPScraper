...
class Submission:
    ...
    def __str__(self):
        if self.site == "codeforces":
            return "Platform: Codeforces, Submission: {}, Contest: {}, Problem: {}, Verdict: {}, When: {}".format(
                self.get_id(),
                self.get_contest(),
                self.get_index(),
                self.get_verdict(),
    ...
    def start(self, cf_autopilot=False, userid=None):
            ...
            if self.codeforces:
                ...
                if self.get_gym and self.cf_password is None:
                    self.cf_password = getpass(
                        "Password is needed for gym contests, please enter password of {} : ".format(
                            self.cf_handle
                        )
                    )
        ...
        if self.codeforces:
            while self.codeforces_once_more:
                if self.verbose:
                    print(
                        "Downloading codeforces submissions for : {} ...".format(
                            self.cf_handle
                        )
                    )
                self.gym_set = set()
                    ...
                    try:
                        ...
                        if self.errors:
                            ...
                            if cf_autopilot:
                                ...
                                self.codeforces_once_more = self.get_input(
                                    "Do you want to run one more time to download these {} submissions ? [y/n]: ".format(
                                        len(self.errors)
                                    )
                                )
                        else:
                        ...
                        if self.verbose:
                            print(
                                "Done downloading codeforces submissions for : {}".format(
                                    self.cf_handle
                                )
                            )
